...
function UserChatRoom() {
  ...
  useEffect(()=>{
    const fetchMessages=async()=>{
      try {
        const response=await Api.get(`/artist/${id}/chatroom/${chatroomId}?role=artist`);
        const data=response.data;
          setMessages(data.messages);  
          setActor({name:data.actor.userName,profile:data.actor.userProfile})
      } catch (err) {
        console.log(err);
      }
  }
...
console.log(file);
 ...
 socketRef.current.emit("send_message", {
   ...
   setMessages((prev) => [
    ...prev,
    { role: "artist", type: "image", image: imageUrl },
  ]);
  } catch (error) {
    console.log(error);
